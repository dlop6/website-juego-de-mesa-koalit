// implementa una simulación de red para el dal que ajusta latencia y errores
...
// lee la configuración desde variables de entorno (públicas y privadas)
const DATA_MODE =
...
// convierte strings a number y devuelve el fallback si no fue válido
...
// interpreta strings comunes como booleano
function toBoolean(value: string | undefined) {
  if (!value) return false;
  return value === "1" || value === "true" || value === "yes";
}

// limita un valor dentro de un rango inclusive
function clamp(value: number, min: number, max: number) {
  return Math.min(max, Math.max(min, value));
}

// calcula la configuración final de la simulación según entorno y env vars
function getSimulationConfig(): SimulationConfig {
  const isTest = DATA_MODE === "test" || NODE_ENV === "test";
  const isDevelopment = NODE_ENV === "development";
  const isProduction = NODE_ENV === "production";

  // define una latencia base diferente para test, dev y prod
  ...
  // aplica jitter solo fuera de test
...
// crea un pequeño wrapper para esperar ms
function delay(ms: number) {
  if (ms <= 0) return Promise.resolve();
  return new Promise((resolve) => setTimeout(resolve, ms));
}

// simula la latencia acumulando latency + jitter y lanza error si estuvo forzado
