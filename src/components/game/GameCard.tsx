...
export function GameCard({
  ...
  return (
    <article
      data-game-id={id}
      className={[
        "group relative flex flex-col bg-surface-dark border border-[#393328] hover:border-primary transition-all duration-300 hover:shadow-[0_0_20px_rgba(242,166,13,0.1)] h-full",
        className,
      ]
        .filter(Boolean)
        .join(" ")}
    >
      <div className="relative w-full aspect-[4/3] sm:aspect-video overflow-hidden border-b border-[#393328] group-hover:border-primary transition-colors">
      ...
      <div className="p-3 sm:p-4 flex flex-col gap-2 sm:gap-3 flex-1">
        <div className="flex justify-between items-start">
          <h3 className="text-white text-base sm:text-lg font-bold leading-tight group-hover:text-primary transition-colors">
            {name}
          </h3>
        ...
        <div className="flex items-center gap-2 mt-auto flex-wrap">
          {themes.map((theme, index) => (
            <span
              key={theme}
              className={[
                "px-2 py-0.5 bg-[#393328] text-[#bab09c] text-[10px] sm:text-xs font-mono rounded-sm",
          ...
          {themes.length > 2 ? (
            <span className="px-2 py-0.5 bg-[#393328] text-[#bab09c] text-[10px] font-mono rounded-sm sm:hidden">
              +{themes.length - 2}
            </span>
          ) : null}
        </div>
        <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-[#393328] mt-2 group-hover:border-primary/30">
          <div className="flex items-center gap-1 text-primary">
            ...
            <span className="text-[10px] sm:text-xs text-[#544c3b]">/5</span>
          </div>
